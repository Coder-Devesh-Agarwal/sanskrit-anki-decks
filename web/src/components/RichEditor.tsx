...
export function RichEditor({ value, onChange }: Props) {
  ...
  function applyTranslit() {
    if (!editor) return
    const { state } = editor
    const repls: { from: number; to: number; text: string; marks: readonly unknown[] }[] = []
    state.doc.descendants((node, pos) => {
      if (node.isText && node.text) {
        const c = transliterate(node.text, inputScheme, outputScheme)
        if (c && c !== node.text)
          repls.push({ from: pos, to: pos + node.text.length, text: c, marks: node.marks })
      }
    })
    ...
    for (let i = repls.length - 1; i >= 0; i--) {
      ...
      tr = tr.replaceWith(r.from, r.to, state.schema.text(r.text, r.marks as any))
    }
  ...
  const showPreview = editor.getText().trim() !== '' && preview !== editor.getText()
    ...
    <div className="rounded border border-slate-700 bg-slate-900">
      {showPreview && (
        <div className="flex items-center gap-2 border-b border-slate-800 bg-slate-800/40 px-2 py-1">
          <span className="text-[10px] uppercase tracking-wide text-slate-500">{outputScheme}</span>
          <span className="dev flex-1 text-sm text-emerald-200">{preview}</span>
        </div>
      )}
      <div className="flex flex-wrap items-center gap-1 border-b border-slate-800 px-2 py-1">
        ...
        <button type="button" onClick={() => editor.chain().focus().toggleItalic().run()} className={`${btn(editor.isActive('italic'))} italic`}>
          I
        </button>
        <button type="button" onClick={() => editor.chain().focus().toggleUnderline().run()} className={`${btn(editor.isActive('underline'))} underline`}>
          U
        </button>
        <button type="button" onClick={() => editor.chain().focus().toggleStrike().run()} className={`${btn(editor.isActive('strike'))} line-through`}>
          S
        </button>
