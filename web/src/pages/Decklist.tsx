  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <Link to="/author" className="rounded bg-sky-600 px-4 py-2 font-medium hover:bg-sky-500">
          + New card
        </Link>
        ...
        <button onClick={() => fileRef.current?.click()} className="rounded bg-slate-700 px-4 py-2 hover:bg-slate-600">
          Import JSON
        </button>
      ...
      {msg && (
        <div className="rounded border border-slate-700 bg-slate-900 p-3 text-sm text-slate-200">{msg}</div>
      )}
        ...
        <ul className="space-y-2">
          {cards.map((c) => (
            ...
            >
              ...
              >
                {c.question || c.finalResult || '(untitled)'}
              </button>
              <span className="text-xs text-slate-500">{c.steps.length} steps</span>
              <Link to={`/author/${c.id}`} className="rounded bg-slate-800 px-2 py-1 text-xs hover:bg-slate-700">
                edit
              </Link>
              <button
                onClick={() => {
